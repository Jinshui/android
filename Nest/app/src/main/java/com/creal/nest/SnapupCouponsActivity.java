...
public class SnapupCouponsActivity extends FragmentActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getSupportFragmentManager().findFragmentById(android.R.id.content) == null) {
            getSupportFragmentManager().beginTransaction().add(android.R.id.content, new CouponListFragment()).commit();
        }
    }

    public static class CouponListFragment extends PTRListFragment<Coupon> {
        ...
        public PaginationAction<Coupon> getPaginationAction() {
            return new CommonPaginationAction(getActivity(), 1, 10, Constants.URL_GET_COUPONS, null, Coupon.class, "coupons");
        }

        public void showDetailActivity(Coupon coupon) {
            if (!coupon.isActive())
                return;
            final Dialog progressDialog = UIUtil.showLoadingDialog(getActivity(), getString(R.string.loading), false);
            Map parameters = new HashMap();
            parameters.put("coupons_id", coupon.getId());
            CommonObjectAction getCouponDetailAction = new CommonObjectAction(getActivity(), Constants.URL_GET_COUPON_DETAIL, parameters, Coupon.class);
            getCouponDetailAction.execute(new AbstractAction.UICallBack<Coupon>() {
                public void onSuccess(Coupon result) {
                    Intent intent = new Intent(getActivity(), SnapCouponDetailActivity.class);
            ...
            if (coupon.isActive()) {
                holder.snapNowBtn.setOnClickListener(new View.OnClickListener() {
                    public void onClick(View v) {
                        ...
                        parameters.put("coupons_id", coupon.getId());
                        CommonObjectAction action = new CommonObjectAction(getActivity(), Constants.URL_RECEIVE_COUPONS, parameters, ReceiveCouponResult.class);
                        action.execute(new AbstractAction.UICallBack<ReceiveCouponResult>() {
                            public void onSuccess(ReceiveCouponResult result) {
                                Intent intent = new Intent(getActivity(), SnapCouponDialogActivity.class);
